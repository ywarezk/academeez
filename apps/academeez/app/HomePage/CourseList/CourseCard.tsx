...
export const CourseCard: FC<{ course: EducationItem }> = ({ course }) => {
  ...
  return (
    <>
      ...
      >
        <CardMedia image={course.bgImg}>
          {isVideo ? (
            <Player className="course-intro-player" ref={player}>
              <HLSSource isVideoChild src={course.videoUrl} />
            </Player>
          ) : (
        ...
        <CardContent>
          ...
          {!isEmpty(course.prerequisites) ? (
            ...
            <ul className="d-flex p-0 flex-wrap list-unstyled">
              <Chip className="mr-1 mb-1" label="None" icon={<Block />} />
            </ul>
          )}
        </CardContent>
        <CardActions>
