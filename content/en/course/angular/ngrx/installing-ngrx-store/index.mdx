...
<p>In production, it's best not to add the `StoreDevtoolsModule` to the imports array, in fact it's best to completly remove the `@ngrx/store-devtools` from the bundle.  
Check out our best practice tip <Link className="text-green/80 underline" href="/en/course/angular/ngrx/best-practices/store-devtools-production">here</Link></p>
</Callout>
