...
<Callout className="mt-3 bg-green/5" title="Best Practice Tip">
	
	In production, it's best not to add the `StoreDevtoolsModule` to the imports array, in fact it's best to completly remove remove the `@ngrx/store-devtools` from the bundle.
