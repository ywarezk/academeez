...
It's recommended to select from the state using <Link className="text-green/80 underline" href="/en/course/angular/ngrx/selectors">`NGRX Selectors`</Link>
</Callout>
...
- timer events
- Effects (which we will discuss about in <Link className="text-green/80 underline" href="/en/course/angular/ngrx/effects">this lesson</Link>)
...
we use `this._store.dispatch(...);` function to send an `Action`, we will have a <Link className="text-green/80 underline" href="/en/course/angular/ngrx/action">full lesson about an action</Link>, but for now let's look at an action as a simple `Object` which describes what happens with an identifier (in our case the identifier is: `type: 'INCREMENT'`) and optional `payload` data if the event needs to send more information.  

<Callout className="my-8 bg-red-30" title="Bad Practice" variant="destructive">
...
![](https://github.com/ywarezk/academeez/blob/main/content/en/course/angular/ngrx/ngrx-store-data-flow/store-dispatch.png?raw=true)
