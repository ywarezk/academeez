...
The way we will structure our repository is that we will work in each lesson in the `main` branch, but in order for student to examine the code in other lessons as well, we will create a branch for every lesson.  
So before starting with lesson 2 in our flux course, let's first make sure that lesson 1 is available in the `lesson-01` branch.
...
This is where things get a bit confusing. There are two different `Kustomization` resources, one from the `kustomize.config.k8s.io/v1beta1` API group, which is client only `kubectl`, and one from the `kustomize.toolkit.fluxcd.io/v1` API group which is a custom resource added to our cluster.
...
`kustomize` kinds of reminds me of a very simplified version of `helm`. You feed kustomize with a resource that looks like so:
...
After installation you will have the `kustomize` command available. This command will not effect your cluster
...
and in the `patch.json` file we can have the following content:
