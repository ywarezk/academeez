...
This video explains how to create a new Angular application and install @ngrx/store, along with setting up Redux DevTools for state inspection.
</span>
...
:::tip[Best Practice Tip]
In production, it's best not to add the `StoreDevtoolsModule` to the imports array, in fact it's best to completely remove the `@ngrx/store-devtools` from the bundle.  
