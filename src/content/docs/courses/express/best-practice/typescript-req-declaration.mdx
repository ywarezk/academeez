...
If we feed the typescript compiler with the declaration files in `node_modules/@types/passport` the compiler is learning now about new types, specifically about the passport module.
...
This option will include a declaration file without the need to specifying it in the source files.
...
If he doesn't include typescript declaration files in his package, the community (or him if he wants to separate the declaration files to a different project) can do it for him and add a declaration file package which can be installed with npm:
...
and when you authenticate with passport, you configure passport to populate the `req.user` with an instance of your `User` class.
...
We can override the `User` interface that is defined by passport to tell typescript that `req.user` is of the type that we want.
