...
- **Asynchronous errors** - those are errors that happen when you perform something asynchronously and that something fails.  
...
you have in the `req` object a reference to the execution context and in there you can feel free to use throw even in async methods or error first callbacks.
...
process.on('uncaughtExceptionMonitor', function(err) {
  ...
  console.log('and we use this event to log the error, create a log file with the error etc.');
});
...
If there is an uncaught exception in express which is not transferred to express error handlers, then that exception will bubble up to the `process` and node will deal with that exception like we learned, by calling the proper events and eventually crashing the node process.
...
The simple example is to have a line of code that can throw an exception, and you can wrap that line of code with a `try..catch` (or equivalent in async types) and deal with the exception at the point of origin.  
...
app.get('/', async (req, res) => {
  try {
    await queryTheDb('some illegal query')
  } catch(err) {
