...
If there is an uncuaght exception in express which is not transferred to express error handlers, then that exception will bubble up to the `process` and node will deal with that exception like we learned, by calling the proper events and eventually crashing the node process.
