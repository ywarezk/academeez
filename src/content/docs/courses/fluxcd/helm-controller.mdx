...
authors:
  - yariv-katz
sources: 
  - url: "https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-03"
...
Regarding **(5)** we will use flux image automation controllers to manage the upgrades - but more on this in a separate lesson.
...
Let's start by subscribing to the `nginx-ingress` repository, since all our clusters will need an ingress controller, we will install it in the `infrastructure/base` folder. 
In that folder create the folder `/infrastructure/base/nginx` where we will place all the resources related to the nginx ingress controller.
Create the file `namespace.yaml` with the following content:

```yaml title="/infrastructure/base/nginx/namespace.yaml"
apiVersion: v1
kind: Namespace
metadata:
  name:  nginx
...
Create the file `infrastructure/base/nginx/repository.yaml` with the following content:

```yaml title="/infrastructure/base/nginx/repository.yaml"
...
Let's add one in the `nginx` folder: `/infrastructure/base/nginx/kustomization.yaml` with the following content:

```yaml title="/infrastructure/base/nginx/kustomization.yaml"
resources:
  - namespace.yaml
  - repository.yaml
```

We will also need a `kustomization.yaml` in the `/infrastructure/base` folder to include the `nginx` folder.

```yaml title="/infrastructure/base/kustomization.yaml"
resources:
  - ./nginx
```

And we will also create a `infrastructure/staging` folder with a `kustomization.yaml` file, which will allow us to patch changes that we want for a specific cluster - in this case the `staging` cluster.

```yaml title="/infrastructure/staging/kustomization.yaml"
resources:
	- ../base
```

It's best to verify using `kustomize` that `kustomize build` of the `/infrastructure/staging` folder will output the expected resources.

```bash
kustomize build infrastructure/staging
```
...
Another thing that we need to do is to tell the `kustomize-controller` to watch the `infrastructure/staging` folder.
We achieve that with a `Kustomization` resource.
Create the file `clusters/staging/infrastructure.yaml` with the following content:

```yaml title="clusters/staging/infrastructure.yaml"
...
In the `infrastructure/base/nginx` folder create the file `chart.yaml` with the following content:

```yaml title="/infrastructure/base/nginx/chart.yaml"
apiVersion: source.toolkit.fluxcd.io/v1beta2
kind: HelmChart
metadata:
  name: nginx
...
But first let's add the chart resource to the `infrastructure/base/nginx/kustomization.yaml` file:
...
In the `infrastructure/base/nginx` folder create the file `release.yaml` with the following content:

```yaml title="/infrastructure/base/nginx/release.yaml"
...
This means we can now delete the chart file: `infrastructure/base/nginx/chart.yaml`.
And update the `infrastructure/base/nginx/kustomization.yaml` file to include the `release.yaml` file:
...
The full source code of this lesson can be found [here](https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-03)
