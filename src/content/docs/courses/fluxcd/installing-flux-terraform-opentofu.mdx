...
sources:
  - title: Installing Flux with Terraform/OpenTofu
    url: https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-04/iac
video:
  videoUrl: https://www.youtube.com/watch?v=68YcfitcKGI
  duration: 5473
...
<div class="not-content flex">
  <iframe
		...
		title="FluxCD - Installing Flux with Terraform/OpenTofu - video"
		allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
		allowFullScreen
		loading="lazy"
		referrerPolicy="strict-origin-when-cross-origin"
		aria-label="FluxCD - Installing Flux with Terraform/OpenTofu - video"
		aria-describedby="iframe-video-description"
  />
</div>

<span id="iframe-video-description" class="sr-only">
This video explains how to install FluxCD on a Kubernetes cluster using Terraform or OpenTofu, covering the creation of a GKE cluster, VPC, subnet, and the configuration of the Flux provider for GitOps.
</span>
...
We will write infrastructure as code and run it with OpenTofu, but the same code can be used with Terraform.  
...
Another important point before we begin is that we will create everything on [GCP](https://cloud.google.com/), if you want an example for other clouds, or you want an example with another infrastructure as code technology, submit an issue (or create a PR if you are up to the challenge).
And don't forget to star the [course repository](https://github.com/Nerdeez/terragrunt-catalog), it helps us a lot.
...
Create a new folder called `iac` and we will place all the infrastructure code there.
...
[terraform-google-modules](https://registry.terraform.io/namespaces/terraform-google-modules) is a collection of terraform/tofu modules created by google.
...
The projects will also help us with isolation, so we will place different environments in different projects, and that isolation will also help us with permissions to different team members.

We will place all the resources in a new project, so let's create a new project on GCP.
Create a new file `iac/main.tf`, we will place all infrastructure code there (another bad practice - no modules no dependencies, this will be fixed in the next lesson).
...
Although the subnet is in a region it doesn't mean that the resources in the subnet are in the exact same location, they can be spread across different zones in that region.
...
Flux can be installed using terraform/tofu. For that we will need to use the [Flux Provider](https://registry.terraform.io/providers/fluxcd/flux/latest/docs).
...
Installing Flux with infrastructure as code (IAC), in our example OpenTofu/Terraform (but other solutions can be used as well) has several advantages:
...
But this sort of authentication can be problematic, mistakes can be made and it can accidentally run not on the cluster that you want it to. Also it will be even harder if multiple team members are using that IAC project.
...
provider "flux" {
	...
	git = {
    url = "https://github.com/Nerdeez/terragrunt-catalog.git"
    http = {
...
Check out the source code [here](https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-04/iac)
