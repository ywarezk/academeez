...
authors:
  - yariv-katz
sources: 
  - url: "https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-02"
...
<div class="not-content flex">
  <iframe
    ...
    title="FluxCD - kustomize-controller and Kustomization resource - video"    
    allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
    referrerPolicy="strict-origin-when-cross-origin"
    aria-label="FluxCD - kustomize-controller and Kustomization resource - video"
    aria-describedby="iframe-video-description"
  />
</div>

<span id="iframe-video-description" class="sr-only">
This video explains the kustomize-controller in FluxCD, detailing how it uses the Kustomization custom resource to manage and synchronize Kubernetes cluster state with resources defined in a kustomization.yaml file, including practical examples and best practices.
</span>
...
For example we might have a base infrastructure configuration between the clusters, but there are some minor configuration changes in that base infrastructure between the clusters, often
...
Move the `clusters/staging/namespace.yaml` to the folder: `apps/base/namespace.yaml` this will represent the namespace where we will place our apps (It's important to note that I usually recommend not to throw all the apps in a single namespace if you have a large project, more logical separation is recommended, but for the sake of simplicity we will use a single namespace for all our apps in this course).
...
The full source code is available [here](https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-02)