...
<div class="not-content flex">
    ...
    title="Git - commit messages - video"
    allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
    referrerPolicy="strict-origin-when-cross-origin"
    aria-label="Git - commit messages - video"
    aria-describedby="iframe-video-description"
	/>
</div>

<span id="iframe-video-description" class="sr-only">
This video explains the importance of writing proper commit messages in Git, introduces the Conventional Commits specification, and demonstrates how to structure commit messages for better readability and automation, with practical examples and best practices.
</span>
...
* **Reviewed-by**: This commit is reviewed by a team member.
...
Notice that we committed the file with the `feat` type, this will bump the version to a minor version, if we would have used a `fix` type it would have been a patch version, and if we had a `BREAKING CHANGE` in the footer it would have been a major version.
...
Since release automation is now being done based on the commit messages, and if we accidentally push a wrong commit message it could affect the release process, it is recommended to force developers
