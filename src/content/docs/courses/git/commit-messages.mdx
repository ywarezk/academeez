...
We can use a tool called [commitlint](https://commitlint.js.org/#/) to enforce commit message rules on a pre-commit hook (we can use [husky](https://typicode.github.io/husky/)).
Another popular solution is to use [commitizen](https://commitizen-tools.github.io/commitizen/) which is a cli tool that helps us write commit messages following rules defined in the project. (we do recommend using a pre-commit hook even if commitizen is used cause it can be bypassed).
