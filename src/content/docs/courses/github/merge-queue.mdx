...
Now let's try and describe a different workflow.
Developer A pushes their PR at 12:00.
Right away he is going to the PR page and he is presented with this option:

![Merge when ready option](merge-queue/merge-when-ready.png)

He doesn't have to wait for the checks to finish, he can schedule the PR to be merged automatically.
When all the requirements for the PR have passed, the PR automatically transfered to a different branch, where the required checks needs to pass on
that branch and the PR will be merged automaticaly

On 12:01 developer B push his PR and also clicks the **Merge when ready**.
Which means the first PR od developer A will be finished at 12:06 and the second PR of developer B will finish running all the checks.
Given that they both pass all the checks they will both be transfered to the branch 
