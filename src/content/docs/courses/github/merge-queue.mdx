...
<div class="not-content flex">
  <iframe
    ...
    width="560"
    height="315"
    src="https://www.youtube.com/embed/vDQ2IG5u1t8?si=QBB644508MhW6vMc"
    title="GitHub Merge Queue - video"
    allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
    referrerPolicy="strict-origin-when-cross-origin"
    aria-label="GitHub Merge Queue - video"
    aria-describedby="iframe-video-description"
  />
</div>
<span id="iframe-video-description" class="sr-only">
  This video is a hands-on walkthrough of GitHub Merge Queue: why pull requests can pass CI yet still fail to merge when the target branch moves, how merge queues run checks against a predicted future state of the branch, and how that fits with branch rulesets and required status checks. It also covers merge groups, ordering, conflicts, important merge queue settings (including requiring all queue entries to pass checks), tradeoffs, and a short comparison with other merge-queue style tools such as Trunk and Mergify.
</span>
...
We will have to enable the **Require merge queue** option, and we can open the collapsible of that option for the following options:
...
the options you choose in **Allowed merge methods** (under the collapsible of **Require a pull request before merging**).
