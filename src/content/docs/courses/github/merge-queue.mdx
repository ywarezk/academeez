...
It solves the problem of developers having to go to the PR page, rebase their branch with the latest main, and run the checks again.
...
Now let's describe a different workflow using GitHub Merge Queue.
...
Here's where it gets interesting: when you hear "queue," you might think that since PR B entered the queue after PR A, it will have to wait in the queue doing nothing. Well, not exactly! In the merge queue, both PRs will run their checks perhaps in parallel (without waiting for each other—this parallelism depends on the merge queue settings), also they might be wrapped in a single branch and the cheks will run once for both of them (depends on the group limits and wait time in the settings). The entrance to the queue does determine the merge order (and there can be some waiting required for that order to be enforced)

The important part is that Developer A and Developer B don't have to worry about rebasing their work with the latest main branch once another PR is merged. Their work will be automatically merged once their checks pass in the queue and all the PRs before them have been merged.
