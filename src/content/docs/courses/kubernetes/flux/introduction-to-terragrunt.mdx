...
In this lesson we will have an introduction to Terragrunt, and using terragrunt we will start arranging the permissions of our IAC project.
 ...
 * will contain common configuration for all modules
 */
 
locals {
  common_vars     = yamldecode(file(find_in_parent_folders("common_vars.yaml")))
  region          = local.common_vars.region
  billing_project = local.common_vars.billing_project
}

/**
 * This provider will authenticate to the gcp using the user's credentials
 */
generate "provider" {
  path      = "provider.tf"
...
- There are certain variables that will repeat in many modules, we are placing those variables in a file called `common_vars.yaml` and we are loading those variables using the `locals` block.
Example of those variables are in the repository in a file called `common_vars.example.yaml` (the `common_vars.yaml` file is in the `.gitignore` file so it won't be commited to the repository).
The `common_vars.yaml` represents variables that will not change, but if needed we can override them by creating an internal `common_vars.yaml` in one of the folders `find_in_parent_folders` will find the first parent `common_vars.yaml`.
...
```hcl
/**
 * create folder using terraform-google-modules/folders/google module
 * we can hardcode the repeating values here 
 */
...
inputs = {
	parent              = "organizations/${local.common_vars.org_id}"
...
```hcl
/**
 * Create a project using terraform-google-modules/project-factory/google module
 * we can hardcode the repeating values for project here
 */
...
terraform {
	source = "tfr:///terraform-google-modules/project-factory/google?version=17.0.0"
}
...
We will use the `_env` modules we just created to create the `common` folder with module for the folder, project, and bucket.
 ...
 * All the resources will be created under this folder
 */
...
// create a folder using the _env/folder module
include "folder" {
	path = "${dirname(find_in_parent_folders())}/_env/folder.hcl"
}

// input to the folder module
inputs = {
...
```hcl
/**
 * The shared folder will contain resources that are shared between the environments
 */
...
// since this folder is under the root, it depends on the root folder creation and will take the output folder id from it
dependency "root" {
	config_path = "../root"
}

inputs = {
	parent = dependency.root.outputs.id
  names  = ["shared"]
}
```
  </TabItem>
...
```hcl
/**
 * Create a project to hold the common resources
 */
...
inputs = {
  ...
  folder_id = dependency.common_folder.outputs.id
}
```
  </TabItem>  
</Tabs>
...
It's recommended to store the state in a remote location, and we will use the bucket we created to store the state.
