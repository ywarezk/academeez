...
<div class="not-content flex">
    ...
    title="Node.js - Is Node.JS Single Threaded? - video"
    allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
    referrerPolicy="strict-origin-when-cross-origin"
    aria-label="Node.js - Is Node.JS Single Threaded? - video"
    aria-describedby="iframe-video-description"
	/>
</div>
<span id="iframe-video-description" class="sr-only">
This video explores the architecture of Node.js to determine whether it is single-threaded, discussing its event loop, thread pool, and how it handles concurrent operations, with practical examples and best practices.
</span>
...
A lot of those standards are defined in the [WHATWG github](https://github.com/whatwg) for example you can find the standard defined for the fetch in the [fetch](https://github.com/whatwg/fetch) repository.
...
- **JavaScript** - is JavaScript single-threaded? Remember JavaScript is a definition of a language as defined by ECMAScript, it's not the actual implementation.
...
What we saw is a starvation of the main thread which caused the server to be stuck, and we solved that by delegating the task to another thread and by doing so freeing the main thread to handle other requests.
...
Opening a thread is something that is not done often in Node.js, the event driven design combined with height performance I/O operations prevents the usage of threads in most cases.
