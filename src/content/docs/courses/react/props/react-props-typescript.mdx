...
We are going the dedicate this lesson to working with Typescript and React, and we will focus on
...
On Javascript that code would pass but if the `<Child />` would do some kind math calculation with the age then that error would have shown during runtime.
...
when using [destructuring](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring) to get the props content which might look like this:
```typescript
export default function Child({ name, age }: { name: string, age: number }) {
...
`ReactNode` is a bit more broad of a type than `JSX.Element`, and you can say that `JSX.Element` is included in `ReactNode`.
...
For the most part you won't need to specify the return type of the component functions, but if you do perhaps it's best to start with a smaller type like `JSX.Element` and expend it to `ReactNode` if the need arises.

Actuallly those types might be more usable in cases where your component is getting a `children` prop, in that case you can specifically assign the `children` as `JSX.Element` if you want the `children` to be a jsx element.
...
Before diving to the exercise we recommend reading about [typescript generic types](https://www.typescriptlang.org/docs/handbook/2/generics.html).
...
So by typescript this code is acceptable:
