...
sidebar:
  order: 8
  label: 4.4 Props Recommendations
video:
  videoUrl: https://www.youtube.com/watch?v=krpGUmiCIEU
  duration: 9095
...
<div class="not-content flex">
	<iframe
		...
		title="React Props Recommendations - video"
		allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
		allowFullScreen
		loading="lazy"
		referrerPolicy="strict-origin-when-cross-origin"
		aria-label="React Props Recommendations - video"
		aria-describedby="iframe-video-description"
	/>
</div>
<span id="iframe-video-description" class="sr-only">
This video provides recommendations for using props in React components, covering best practices for prop naming, type definitions, avoiding prop drilling, and optimizing component performance, with practical examples and tips.
</span>
...
That language you create enforces you to build your application like lego blocks using good infrastructure, and reusing your code.
...
For example in your database, in the tables columns, strive to use similar naming for similar columns, if for example a certain **entity** (A) contains a **name** column, and another **entity** (B) contains also a **name** keeping the same conventions will direct you to place the column for **B** that is named `name` and not for example using in **A** the column `name` and in **B** the column `title`.  
...
Generics are just a subset of the power of typescript which I see a lot of experienced developers miss out when writing their components.
...
1. When the component you are transferring the props to is a pure component that is wrapped with `React.memo`, and the components you are passing are non primitives, and you want to avoid rendering this component when the parent renders:
...
Seems like such a simple topic, yet I see so much bad practices and mistakes of even the most experienced `React` developers.
