...
This video provides a comprehensive overview of React Suspense, explaining its purpose and how it can be used to manage asynchronous operations in React applications, with practical examples and best practices.
</span>
...
- If the `thrown` data is an instance of `Promise` it will be caught by the first `<Suspense>` up the tree
- All other types or promise instance with no `<Suspense>` to catch it will be caught by the first `<ErrorBoundary>` up the tree (We will talk on `ErrorBoundary` in future lessons)

If there is no one to catch the thrown data, the usual Javascript uncaught exception will happen (usually resulting in a crash).
