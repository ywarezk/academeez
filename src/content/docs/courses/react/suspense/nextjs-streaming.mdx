...
<div class="not-content flex">
    ...
    title="React Suspense and NextJS Streaming - video"
    allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    allowFullScreen
    loading="lazy"
    referrerPolicy="strict-origin-when-cross-origin"
    aria-label="React Suspense and NextJS Streaming - video"
    aria-describedby="iframe-video-description"
	/>
</div>
<span id="iframe-video-description" class="sr-only">
This video explains how to use React Suspense for streaming data in NextJS applications, demonstrating techniques for improving performance and user experience with server-side rendering and progressive content loading.
</span>
...
The backend evaluates `<Suspense>` in a streaming frame, it will evaluate the `<Suspense>` promises, and if they are pending (at least one) it will stream the fallback as a streaming HTML frame, if all the promises are resolved it will stream the children, and if at least on promise is rejected it will stream the content of the error-boundary (or uncaught exception if there is no error-boundary).
