...
sidebar:
  order: 7
sources: 
  - url: "https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-07"
    title: "IAM example"
---
...
Using the impersonation tricks we will create several service accounts with different gcp resource creation privileges, we can then give impersonation permissions to our groups to impersonate certain service accounts according to the permissions we want to grant the team members in the group.
...
* Enhanced security - we will follow the principle of least privilege, every user will get the minimum amount of permissions required.
...
You can find the permissions [here](https://cloud.google.com/iam/docs/permissions-reference) and you find the relevant ones by searching using the service permission prefix.
...
At the moment our root Terragrunt looks like [this](https://github.com/Nerdeez/terragrunt-catalog/blob/lesson-06/iac/gcp/tofu/terragrunt.hcl): 
...
Notice that we are not only giving the minimal permissions, we are also carefully assigning them to the minimal resource that requires them, for example I won't place the role `roles/iam.serviceAccountAdmin` on the organization allowing to create service accounts everywhere,
...
But what we can do is assign the common `iac/gcp/tofu/common` to the devops group, by creating `sa.yaml` file in the `iac/gcp/tofu/common` folder:
