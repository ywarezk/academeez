...
נתחיל בהרשמה ל - `nginx-ingress` repository, מאחר וכל הקלאסטרים שלנו יצטרכו להתקין ingress controller, נתקין אותו בתיקיית `infrastructure/base` .
בתוך התיקייה ניצור תיקייה נוספת `/infrastructure/base/nginx` שבה נציב את כל המשאבים הקשורים ל - nginx ingress controller.
...
ניצור את הקובץ `infrastructure/base/nginx/repository.yaml` עם התוכן הבא:
...
נוסיף אחד בתיקיית `nginx`: `/infrastructure/base/nginx/kustomization.yaml` עם התוכן הבא:
...
נצטרך גם קובץ `kustomization.yaml` בתיקיית `/infrastructure/base` שיכלול את התיקייה `nginx`.
...
בנוסף ניצור תיקייה `/infrastructure/staging` עם קובץ `kustomization.yaml` שיאפשר לנו לעשות patch לשינויים שאנו רוצים להחיל על קלאסטר מסוים - במקרה זה קלאסטר `staging`.
...
כדאי לוודא עם `kustomize` שהפקודה `kustomize build` של התיקייה `/infrastructure/staging` תחזיר את המשאבים הרצויים.
...
דבר נוסף שעלינו לעשות הוא להודיע ל - `kustomize-controller` לצפות בתיקיית `infrastructure/staging`.
את זאת באמצעות `Kustomization` resource של Flux.
ניצור את הקובץ `clusters/staging/infrastructure.yaml` עם התוכן הבא:
...
בתיקיית `/infrastructure/base/nginx` ניצור את הקובץ `chart.yaml` עם התוכן הבא:
...
ראשית נוסיף את ה - `chart` ל - `infrastructure/base/nginx/kustomization.yaml`:
...
בתיקייה `/infrastructure/base/nginx` ניצור את הקובץ `release.yaml` עם התוכן הבא:
...
כלומר אנחנו יכולים למחוק את הקובץ `infrastructure/base/nginx/chart.yaml`.
ולערוך את הקובץ `infrastructure/base/nginx/kustomization.yaml` שיכלול את הקובץ `release.yaml`:
...
קוד המקור המלא של שיעור זה ניתן למצוא [כאן](https://github.com/Nerdeez/terragrunt-catalog/tree/lesson-03)
