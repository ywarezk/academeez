...
אם ננסה לשלוח את כל הנתונים מה - `<Parent />` ל - 50 components האלו זה יהיה מסובך, נצטרך ליצור משהו בסגנון `Context` כדי להעביר את הנתונים, והעובדה שה - `<Parent />` צריך להתמודד עם המון נתונים שלא באמת קשורים אליו (למעט כאשר להציג את הספינר) לא עושה את הקוד שלנו להיראות יותר טוב (גם פוגע ב - single responsibility של ה - `<Parent />`).
